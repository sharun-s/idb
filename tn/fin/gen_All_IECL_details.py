...
def SubDeptsBreakup(titleStr, head, indexfile, detailsdir):
	...
	if pp(d,v,head,titleStr,detailsdir):
		updateIndex(head,format_indian(ta*1000),indexfile,detailsdir)
		#print('done',head)
...
for f in revfiles:
	ppIncomeByDepts(f'data/revenue/breakup/{f}', f[:4])
	updateIndex(f[:4],'','income_index.html','income_explorer')
	print('done',f)