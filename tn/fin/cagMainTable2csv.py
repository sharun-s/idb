...
for i in allfiles:
	...
	if firsttime:
		labels=tables[0].df.iloc[startrow:-1][0].append(tables[1].df.iloc[0:][0]).append(p.Series(['Year','Month']))
		p.DataFrame(labels.str.replace('\n',',')).T.to_csv(outfile,mode='a',header=False,index=False)
		firsttime=False
	if Month.find('4')>-1:
		estimate=tables[0].df.iloc[startrow:-1][1].append(tables[1].df.iloc[0:][1]).apply(lambda x:x.replace(' ','')).apply(lambda x:atof(x) if x and x!='..' and x!='$' else None).append(p.Series([int(Year),9999]))
		p.DataFrame(estimate).T.to_csv(outfile,mode='a',header=False,index=False)

	actual=tables[0].df.iloc[startrow:-1][2].append(tables[1].df.iloc[0:][2]).apply(lambda x:x.replace(' ','')).apply(lambda x:atof(x) if x and x!='..' and x!='$' else None).append(p.Series([int(Year),int(Month)]))
	assert int(actual.iloc[1])== int(actual.iloc[2:9].sum())
