...
for s in range(0,len(states)):
	...
	df=p.read_csv(r'data/cag/csv/'+states.prefix.iloc[s]+sys.argv[1],comment='#')
	if len(df) == 0:
...
cats=['Expenditure','Interest Payments','Wages','Pensions','Subsidies','CapEx','LoansGiven']
colors=["#ffc107","#33ccff",'#F95733','#00ff88','#F9e792',"#ff1122",'#A6E22E']
outfile='expenditure.png'
for i in range(0,len(cats)):
	fig = plt.figure(facecolor="#001f3f",figsize=(8.,6.4))
	fig.suptitle('Southern States Expenditure Breakup FY19 (upto Jan)', x=0.35,color="#E6DB74", fontsize=16)
	...
	l=ax.legend(loc='lower left', ncol=4, bbox_to_anchor=(-0.35, .97), frameon=False, facecolor='none')
