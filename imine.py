...
cc=mine(bb) #linkbacks
dd={} #allcounts
for i in cc:
    for j in cc[i]:
    if dd.has_key(i):
        dd[i].append((j,len(cc[i][j])))
    else:
        dd[i]=[(j,len(cc[i][j]))]
...
#from movies as this is every entry in malli is a dict not an Ad which has a dict
def mine(self, lobjs, ignore=[]):
    results={}
    idx=0
    for obj in lobjs:
        #print idx,
            ...
            if prop not in ignore:
                if results.has_key(prop):
                    if type(obj[prop]) is list:
                        ...
                        if results[prop].has_key(obj[prop]):
                            results[prop][obj[prop]].append(idx)
                        else:
                            results[prop].update({obj[prop]:[idx]})
                else:
                    #adding neew prop eg: results['location']= {'adyar':[3]} where 3 is a index into listof ads
                    if type(obj[prop]) is list:
                        l=obj[prop]
